...
function FavoritesPage({rooms}: FavoritesPageProps): JSX.Element {
  ...
  const getPlaces = (city :string) => favoriteRooms.filter((room) => room.city.name === city).map((room) =>
    <FavoritePlace key={room.id} room={room}/>);

  const getCities = () => favoriteCities.map((city) => (
